...
typecheckGlobalDefinition (Rename.GlobalDefinitionValue (Rename.GlobalValueDefinition symbol type_ expression loc)) = do
  ...
  when (type_ /= expressionType) $ throwError ("definition of \"" ++ show symbol ++ "\": expected " ++ show type_ ++ " but got " ++ show expressionType, Just loc)
...
typecheckGlobalDefinition (Rename.GlobalDefinitionFunction (Rename.FunctionDefinition symbol parameters resultType expression loc)) = do
  ...
  when (resultType /= expressionType) $ throwError ("definition of \"" ++ show symbol ++ "\": expected " ++ show resultType ++ " but got " ++ show expressionType, Just loc)
...
typecheckExpression (Rename.Call symbol arguments loc) = do
  ...
  when (numberArgumentsExpected /= numberArgumentsPassed) $ throwError ("call of function " ++ show symbol ++ ": wrong number of arguments. expected: " ++ show numberArgumentsExpected ++ ", passed: " ++ show numberArgumentsPassed, Just loc)
  ...
  return (Call symbol typedArguments resultType loc, resultType) where
    ...
    typecheckParameter (parameterType, argumentType) = when (parameterType /= argumentType) $ throwError ("call of function" ++ show symbol ++ ": expected " ++ show parameterType ++ " but got " ++ show argumentType, Just loc)
...
typecheckExpression (Rename.If conditionExpression trueExpression falseExpression loc) = do
  ...
  when (conditionExpressionType /= TypeBoolean) $ throwError ("condition expression must be of type " ++ show TypeBoolean ++ " but has type " ++ show conditionExpressionType, Just loc)
  ...
  when (trueExpressionType /= falseExpressionType) $ throwError ("types of if branches differ: " ++ show trueExpressionType ++ " and " ++ show falseExpressionType, Just loc)
