...
compile astModule = withContext $ \context -> do
  withModuleFromAST context astModule $ \llvmModule -> do
    withHostTargetMachine $ \targetMachine -> do
      ...
      callProcess "ld" ["-demangle", "-dynamic", "-arch", "x86_64", "-macosx_version_min", "10.14.0", "-lSystem", "/usr/local/opt/llvm@6/lib/clang/6.0.1/lib/darwin/libclang_rt.osx.a", objectFilePath, inBuildFolder "builtins.o", "-o", inBuildFolder moduleName]
